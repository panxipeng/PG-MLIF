...
data_cv_path = '%s/splits/gbmlgg15cv_%s_%d_%d_%d%s_%s.pkl' % (opt.dataroot, roi_dir, ignore_missing_moltype, ignore_missing_histype, opt.use_vgg_features, use_rnaseq,'2splits_15')
...
for k, data in data_cv_splits.items():
	...
	if opt.task == 'surv':
		print("[Final] Apply model to training set: C-Index: %.10f, Acc: %.10f, P-Value: %.10e" % (cindex_train,  surv_acc_train, pvalue_train))
		logging.info("[Final] Apply model to training set: C-Index: %.10f, Acc: %.10f, P-Value: %.10e" % (cindex_train, surv_acc_train, pvalue_train))
...
print("C-index Average:", np.array(results).mean())
